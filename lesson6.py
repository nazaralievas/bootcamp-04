...
# для удаления элемента (если хотим удалить студентку Kate из списка) можно воспользоваться методом remove:
students.remove('Kate') # в скобочках пишем что именно мы хотим удалить
# и кстати если у нас в списке две студентки с именем Kate, то он удалит только первую встретившуюся Kate в списке

# или можно воспользоваться методом pop()
deleted_student = students.pop(0) # указываем индекс элемента который хотим удалить
# pop() позволяет нам не просто удалить элемент, но и сохранить его в переменной

# если примените метод pop() просто с пустыми скобочками, то он удалит вам последний элемент с списке
students.pop()